...
def render_streamlit(data_dir: Path):
    ...
    try:
        ...
        # 挑選正確的檔案 (過濾條件：有累計核發，沒有初次申請)
        candidates = [
            p for p in excel_files
            if ("累計核發" in p.name) and ("初次申請" not in p.name)
        ]
        ...
        if not src_path:
            st.error("找不到符合規格的來源檔 (需含：累計核發、統計年月、總計，且不含初次申請)")
            return
        ...
        # ✨ 將原本要寫入 Word 的表格，直接渲染在網頁上！
        st.dataframe(table1_df, hide_index=True, width="stretch")
